...
public class AddTaskActivity extends AppCompatActivity {
    ...
    public void onClickAddTask(View view) {
        ...
        if(!editText.getText().toString().isEmpty()){
            ...
            cv.put(TaskContract.TaskEntry.COLUMN_DESCRIPTION, editText.getText().toString());
            cv.put(TaskContract.TaskEntry.COLUMN_PRIORITY, mPriority);

            ContentResolver contentResolver = getContentResolver();
            Uri returnUri = contentResolver.insert(TaskContract.TaskEntry.CONTENT_URI, cv);

            if(null != returnUri)
                Toast.makeText(this, returnUri.toString(), Toast.LENGTH_LONG).show();
